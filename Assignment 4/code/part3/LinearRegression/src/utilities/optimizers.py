...
def shuffle(y, x):
    ...
    y = np.array([z[0] for z in zipped])
    x = np.array([z[1] for z in zipped])
    return y, x
...
def mini_batch_gradient_descent(y, x, theta, max_iters, alpha, metric_type, mini_batch_size):
    ...
    num_of_samples = len(x)
    num_of_batches = ceil(num_of_samples / mini_batch_size)
    for i in range(max_iters):
        y, x = shuffle(y, x)
        for m in range(num_of_batches):
            # This is for MSE loss only
            x_n = x[m * mini_batch_size: m * mini_batch_size + mini_batch_size + 1]
            y_n = y[m * mini_batch_size: m * mini_batch_size + mini_batch_size + 1]

            gradient = -2 * x_n.T.dot(y_n - x_n.dot(theta)) / mini_batch_size
            theta = theta - alpha * gradient
